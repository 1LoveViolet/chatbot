...
# 保存词到ID的映射，同时也保存反向的从ID到词的映射，还记录每个词出现的次数，以及总共出现的词的个数
class Voc:
...
# 预处理使用函数unicodeToAscii来把unicode字符变成ascii，
# 比如把à变成a如果是中文，这个函数直接会丢弃掉。
...
def evaluateInput(encoder, decoder, searcher, voc):
    ...
    while (1):
        try:
            with open("D:\chatBot\chat.txt", "r") as file:
                # 读取文件内容
                text = file.read()
            if text != input_sentence:
                input_sentence = text
            ...
            botAnswer = ' '.join(words)
            with open("botAnswer.txt", "w") as file:
                # 写入文本内容
                file.write(botAnswer)
            print("文本已成功保存到文件。")
        except KeyError:
...
checkpoint = torch.load('data/save/cb_model/cornell movie-dialogs corpus/{}-{}_{}/4000_checkpoint.tar')
